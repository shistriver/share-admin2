...
const RichTextForm: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // 这里可以添加实际提交逻辑
      // await api.submitContent(values.content);
    } catch (error) {
  ...
  return (
    <div>
      <h2 style={{ marginBottom: 20, textAlign: 'center' }}>发布文章</h2>
      <Form
        form={form}
        name="nest-messages"
        labelCol={{ span: 4 }}
        ...
        initialValues={{ status: '0', visibility: '1', is_featured: '0', point_threshold: 0 }}
      >
        ...
        <Form.Item name="cover_image_url" label="封面图" rules={[{ required: true }]}>
        ...
        <Form.Item name="visibility" label="可见性">
          <Select>
            ...
            <Select.Option value="1">仅自己可见</Select.Option>
            <Select.Option value="2">密码保护</Select.Option>
          </Select>
        </Form.Item>
        <Form.Item name="is_featured" label="推荐">
          <Radio.Group>
        ...
        <Form.Item name="point_threshold" label="下载积分" rules={[{ required: true }]}>
